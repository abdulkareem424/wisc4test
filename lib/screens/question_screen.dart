...
class _QuestionScreenState extends State<QuestionScreen> {
  ...
  Future<void> _loadQuestions() async {
    try {
      final raw = await rootBundle.loadString('assets/questions/${widget.subtest.toLowerCase()}.json');
      final arr = jsonDecode(raw) as List<dynamic>;
      questions = arr.map((e) => Question.fromMap(e as Map<String, dynamic>)).toList();
      if (questions.isNotEmpty) {
        setState(() { remaining = questions.first.timeLimit; });
        _startTimer();
      } else {
        setState(() {});
      }
  ...
  Future<void> _recordScore(int score) async {
    ...
    final sessionId = await db.createSession(widget.childMap != null ? widget.childMap!['id'] as int : 0, widget.subtest);
    await db.insertAnswer({'sessionId': sessionId, 'questionId': questions[idx].id, 'score': score, 'timeTaken': timeTakenForCurrent});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text('${widget.subtest} - سؤال ${idx+1}/${questions.length}')),
      body: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text('الوقت المتبقي: \$remaining ثانية', style: const TextStyle(fontSize: 16)),
            const SizedBox(height: 20),
            Wrap(
              spacing: 10,
            ...
            const Spacer(),
            ElevatedButton(onPressed: () => _recordScore(0), child: const Text('تخطي / صفر'))
          ],
        ),
      ),
    );
